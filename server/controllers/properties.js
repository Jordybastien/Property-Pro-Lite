...
export const getPropertiesByType = async (req, res) => {
  let proptype = await client.query('SELECT * FROM properties WHERE type=$1',[
    req.params.type
  ]);
    if (proptype.rows.length>0){
      
      return responses.response(res,200, proptype.rows,false);
    done();
...
export const deleteProperty = (req, res) => {
    //Delete 
    let deleteprop = client.query('DELETE FROM properties WHERE id =$1',[
     req.params.id
    ]);
    if (!deleteprop){
      return responses.response(res, 404, 'Error running query',true);
...
export const propertyIsSold = (req, res) => {
    ...
    let updateprop = client.query('UPDATE properties SET status=$1 where id = $2',[
     ...
     .then(() =>{
      return responses.response(res,201,'Mark as sold',false);  
      done();
     });
